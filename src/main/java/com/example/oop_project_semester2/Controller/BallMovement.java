...
/**
 * This class controls the movement of the ball.
 */
public class BallMovement {
     ...
     * @param pongball      the ball
     * @param racket1       the left racket
     * @param racket2       the right racket
    ...
    public void startBallMovementThread(Scene scene, ImageView pongball, Rectangle racket1, Rectangle racket2,
        ...
        if (!isGameEnded(player1, player2)) {
            Thread ballMovementThread = new Thread(() -> {
                ...
                // Variable to track the direction the ball is spinning
                float direction = 1;
                // Variable to multiply the speed of the ball based on the number of times it bounces
                double speedIncreaseFactor = 1.1;
                int speedIncreaseCounter1 = 0; // Counter for tracking speed increase for every bounce
                int speedIncreaseCounter2 = 0; // Counter for tracking speed increase every second bounce
                int speedIncreaseCounter3 = 0; // Counter for tracking speed increase every third bounce

                // Variable to set a max speed that the ball can go
                final double MAX_SPEED = 8.0;

                // Ball movement loop
                while (true) {
                    // If the game is not paused move the ball, otherwise stop
                    if (!isPaused) {
                        // The new position of the ball at X and Y
                        ...
                        if (newBallY <= 100 || newBallY >= scene.getHeight() - pongball.getFitHeight()) {
                            ...
                            // If the ball is set to increases speed every bounce, increase its speed every bounce
                            if (ballSpeed.getSpeedIncrease() == 1) {
                                speedIncreaseCounter1++;
                                ballXSpeed *= speedIncreaseFactor;
                                ballYSpeed *= speedIncreaseFactor;
                            }
                            // If the ball is set to increases speed every second bounce, increase its speed every second bounce
                            ...
                            // If the ball is set to increases speed every third bounce, increase its speed every third bounce
                        ...
                        if (CollisionDetection.isCollidingWithRacket(pongball, racket1)) {
                            ...
                            direction *= -1;// Reverse the direction the ball is spinning
...
// Method to resume the ball movement thread
    ...
    /**
     * Restart the ball on restart.
     *
     * @param pongball      the ball image
     * @param ball          the ball
     * @param scene         the scene
     * @param winnerMessage the winner message
     */
    public void restartBall(ImageView pongball, Ball ball, Scene scene, Text winnerMessage) {
        // Reset ball position to center
        ...
        Platform.runLater(() -> winnerMessage.setText(""));
    }
    /**
